...
def generate():
    ...
    with col1:
        task = st.text_input("Task Description", "Write a science fiction novel", label_visibility="collapsed")
    ## A button to analyze the task and generate the modules
    with col2:
        if st.button("Analyze Task", type="primary"):
            ## Get the modules
            state.module_messages = [{"role": "user", "content": f"The task I want LLM to perform is: {task}"}]
            state.modules = get_modules(state.generator, state.module_messages)

    with st.sidebar:
        st.subheader("Basic Information")
        state.role_name = st.text_input("Assistant Name", "", help="For example: Large Model, Assistant, etc.")
        state.author = st.text_input("Author", "LangGPT")
        state.version = st.number_input("Version", min_value=0.1, value=0.1, step=0.1)
        state.description = st.text_area("Description", "This is an assistant generated by LangGPT", height=100)
    ...
    if "modules" in state:
        ...
        if state.on_modules["style"]:
            st.subheader("Please specify the response style:")
            style = st.text_input("Style", "", help="For example: formal, humorous, serious, etc.", label_visibility="collapsed")
            state.style = style
